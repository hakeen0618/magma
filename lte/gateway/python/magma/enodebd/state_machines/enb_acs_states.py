...
class SendDownloadState(EnodebAcsState):
    def __init__(self, acs: EnodebAcsStateMachine, when_done: str):
        super().__init__()
        self.acs = acs
        self.done_transition = when_done
    ...
    def state_description(self) -> str:
        return 'Running Download to eNB'
...
class WaitDownloadResponseState(EnodebAcsState):
    ...
    def get_msg(self, message: Any) -> AcsMsgAndTransition:
        """ Reply with empty message """
        logger.info("this is download message")
        logger.info(message)
        return AcsMsgAndTransition(models.DummyInput(), self.done_transition)
    def state_description(self) -> str:
        return 'Running download to eNodeB'
...
class WaitDownloadDelayState(EnodebAcsState):
    """
    After receiving the Inform notifying us that the eNodeB has successfully
    ...
    def state_description(self) -> str:
        return 'Waiting after eNB download to prevent race conditions'
